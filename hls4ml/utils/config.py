...
def config_from_keras_model(model, granularity='model', default_precision='ap_fixed<16,6>', default_reuse_factor=1):
    ...
    conv_layers = ['Conv1D', 'Conv2D', 'BinaryConv2D', 'SeparableConv2D', 'DepthwiseConv2D']
    ...
    supported_layers = core_layers + dense_layers + conv_layers + pooling_layers + norm_layers + activation_layers\
    + merge_layers + qkeras_layers + upsampling_layers + reshaping_layers + graph_layers + rnn_layers + deepcalo_layers + skip_layers
